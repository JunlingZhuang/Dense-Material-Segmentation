...
def process_segmented_images_and_fill_to_csv(csv_path, segmented_images_dir, segmented_images_output, lookup_table_path):
    with open("/Users/zhuangjunling/Documents/GitHub/GSAPP/Dense-Material-Segmentation/Flask/Columbia_SVIpoints_4326_output.json", "r", encoding="utf-8") as file:
        data = json.load(file)
    # print(data)
    # 读取csv文件
    ...
    for image_name in segmented_images:
        ...
        # print(row_idx)
        material_percent_dict = {material_name: 0 for material_name in material_names}
        for material_name, percentage in materials_percent.items():
            ...
            # 将数据添加到字典中
            material_percent_dict[material_name] = f"{percentage:.2f}"
        # 将字典添加到 JSON 对象（Python 列表）中
        data.append(material_percent_dict)
        # 输出 JSON 格式
        material_percent_json = json.dumps(material_percent_dict, indent=4, ensure_ascii=False)
        print(material_percent_json)
    ...
    # 保存csv文件
    new_csv_path = '/Users/zhuangjunling/Documents/GitHub/GSAPP/Dense-Material-Segmentation/Flask/Data/Columbia_SVIpoints_4326_output.csv.csv'
...
segmented_images_output = '/Users/zhuangjunling/Documents/GitHub/GSAPP/Dense-Material-Segmentation/Flask/output'

segmented_images_dir = '/Users/zhuangjunling/Documents/GitHub/GSAPP/Dense-Material-Segmentation/Flask/input'
segmented_images = [f for f in os.listdir(segmented_images_dir) if f.endswith('.jpg') or f.endswith('.png')]
csv_path = '/Users/zhuangjunling/Documents/GitHub/GSAPP/Dense-Material-Segmentation/Flask/Data/Columbia_SVIpoints_4326.csv'
